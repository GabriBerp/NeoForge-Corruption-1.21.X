...
import com.gabriberp.corruption.Entity.Client.Animations.SentinelEggAnimations;
...
public class SentinelEggModel<T extends SentinelEggEntity> extends HierarchicalModel<T> {
    // This layer location should be baked with EntityRendererProvider.Context in the entity renderer and passed into this model's constructor
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(CorruptionMod.MOD_ID, "sentinel_egg"), "main");
    private final ModelPart Lower_body;

    public SentinelEggModel(ModelPart root) {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition lower_body = partdefinition.addOrReplaceChild("lower_body", CubeListBuilder.create().texOffs(0, 0).addBox(-5.326F, 3.3901F, -8.5F, 15.0F, 4.0F, 16.0F, new CubeDeformation(0.0F))
                ...
                .texOffs(58, 20).addBox(-9.326F, 4.3901F, -5.5F, 14.0F, 3.0F, 11.0F, new CubeDeformation(0.0F)), PartPose.offset(-0.674F, 16.6099F, -0.5F));
    ...
    @Override
    public void setupAnim(@NotNull SentinelEggEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);

        this.animate(entity.idleAnimationState, SentinelEggAnimations.SENTINEL_IDLE, ageInTicks, 1f);
    }
